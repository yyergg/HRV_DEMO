...
public class BLEManager {
    ...
    public ArrayList<Integer> HR_array;
    ...
    public BLEManager(Activity activity){
        ...
        this.HR_array = new ArrayList<Integer>();
        for(int i=0;i < 500;i++){
            this.HR_array.add(70);
        }
    }
    ...
    private void broadcastUpdate(final String action,
        ...
        if (characteristic.getUuid().toString().substring(0, 8).equals(HEART_RATE_MEASUREMENT)) {
            ...
            if(RrInterval){
                RRIntervalValue = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT16, 1);
            }
            ...
            if(this.HR_array.size() > 500){
                this.HR_array.remove(0);
            }
            this.HR_array.add(HR_amount);
